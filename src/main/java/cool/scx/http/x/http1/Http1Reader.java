...
public final class Http1Reader {
    ...
    public static ByteSupplier readBodyByteInput(Http1Headers headers, ByteInput byteInput, long maxPayloadSize) throws ContentTooLargeException {
    ...
    public static Http1Headers readHeaders(ByteInput byteInput, int maxHeaderSize) throws RequestHeaderFieldsTooLargeException, CloseConnectionException {
        //尝试读取 headers
        try {
            // 1, 尝试检查空头的情况 , 即请求行后紧跟 \r\n
    ...
    public static Http1RequestLine readRequestLine(ByteInput dataReader, int maxRequestLineSize) throws URITooLongException, InvalidHttpRequestLineException, HttpVersionNotSupportedException, CloseConnectionException {
        //尝试读取 请求行
        try {
            // 1, 尝试读取到 第一个 \r\n 为止
        ...
        } catch (InvalidRequestLineException e) {
            // 解析 RequestLine 异常
            throw new InvalidHttpRequestLineException("Invalid HTTP request line : " + e.requestLineStr);
        } catch (InvalidRequestLineHttpVersionException e) {
            // 错误的 Http 版本异常
            throw new HttpVersionNotSupportedException("Invalid HTTP version : " + e.httpVersionStr);
        }
    }
    ...
    public static Http1StatusLine readStatusLine(ByteInput byteInput, int maxStatusLineSize) {
        ...
        } catch (InvalidStatusLineException | InvalidStatusLineStatusCodeException |
                 InvalidStatusLineHttpVersionException e) {
            // 同上, 解析异常我们全部包装到 RuntimeException 中
